...
if ($empRole != Vendor) {
  ...
  $getAllItr = getAllItrById($_SESSION['AdminLogin']);
}
              ...
              ?>
                <tr>
                  ...
                  <td>
                    ...
                    <br>
                    <?php
                    if (!empty($rows['salary_sheet'])) {
                      $filenames = explode(',', $rows['salary_sheet']);
                      foreach ($filenames as $key => $filename) {
                        $filename = trim($filename);
                        $filePath = SITEPATH . 'upload/Images/' . $filename;
                    ?>
                        <a href="<?php echo $filePath; ?>" target="_blank">View Salary Sheet-<?php echo ++$key; ?></a><br>
                    <?php
                      }
                    }
                    ?>
                    <?php
                    if (!empty($rows['itr_bank_statement'])) {
                      $filenames = explode(',', $rows['itr_bank_statement']);
                      foreach ($filenames as $key => $filename) {
                        $filename = trim($filename);
                        $filePath = SITEPATH . 'upload/Images/' . $filename;
                    ?>
                        <a href="<?php echo $filePath; ?>" target="_blank">View ITR Bank Statement-<?php echo ++$key; ?></a><br>
                    <?php
                      }
                    }
                    ?>
                  ...
                  <td id="font12" style="width:15%">
                    ...
                    <?php if ($per['itr_management']['edit'] == 1 && ($empRole == Admin || $empRole == Administrative)) : ?>
                      <?php if ($rows['status'] >= 0) : ?>
                        <a href="javascript:void(0)" onclick="uploadFiles(<?= $rows['itr_id'] ?>, '<?= $getCustomerDetails[0]['cust_first_name']; ?>')" onMouseOver="showbox('Upload<?= $i; ?>')" onMouseOut="hidebox('Upload<?= $i; ?>')"> <i class="fa fa-upload"></i> </a>
