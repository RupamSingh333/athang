...
switch ($action) {
    case "save":
        ...
        $field = array();
        $field['plan_name'] = get_safe_post('plan_name');
        ...
        if ($_FILES["single_images"]["error"] == 0) {
            ...
            if ($primary_value) {
  			...
  			$output = save_command(tbl_plans, $field, "plan_id", $primary_value);
            $_SESSION['msg'] = $output;
    ...
    case "del":
        ...
        if ($primary_value) {
            ...
            $output = del_command(tbl_plans, "plan_id", $primary_value, false);
            $_SESSION['msg'] = $output;
            $_SESSION['alert_type'] = 'success';
        } else {
