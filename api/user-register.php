...
function handleFileUpload($fileKey, $columnName, $userId)
{
    ...
    if (!empty($_FILES[$fileKey]["name"]) && $_FILES[$fileKey]["error"] == 0) {
        $findUser = getuser_byID($userId);
        $file_name = $_FILES[$fileKey]["name"];
        $file_ext = strtolower(pathinfo($file_name, PATHINFO_EXTENSION));
        $customer_img_name = $fileKey . '_' . rand() . '_' . time() . "_" . strtolower(str_replace(" ", "_", $findUser['first_name'] . '.' . $file_ext));
        $path = '../' . $config['category_thumb'] . $customer_img_name;

        if (move_uploaded_file($_FILES[$fileKey]["tmp_name"], $path)) {
            unlink('../' . $config['category_thumb'] . $findUser[$columnName]);
